...
HanziWriter.prototype.showOutline = function () {
  ...
  return this._withData(function () {
    return _this7._renderState.run(characterActions.showCharacter('outline', _this7._character, typeof options.duration === 'number' ? options.duration : _this7._options.strokeFadeDuration)).then(function (res) {
      return callIfExists(options.onComplete, res);
    });
  });
};
...
HanziWriter.prototype.hideOutline = function () {
  ...
  return this._withData(function () {
    return _this8._renderState.run(characterActions.hideCharacter('outline', _this8._character, typeof options.duration === 'number' ? options.duration : _this8._options.strokeFadeDuration)).then(function (res) {
      return callIfExists(options.onComplete, res);
    });
  });
};
...
HanziWriter.prototype.updateColor = function (colorName, colorVal) {
  ...
  return this._withData(function () {
    var duration = typeof options.duration === 'number' ? options.duration : _this9._options.strokeFadeDuration;
    var fixedColorVal = colorVal;
    // If we're removing radical color, tween it to the stroke color
    if (colorName === 'radicalColor' && !colorVal) {
      fixedColorVal = _this9._options.strokeColor;
    }
    var mappedColor = colorStringToVals(fixedColorVal);
    _this9._options[colorName] = colorVal;
    ...
    return _this9._renderState.run(mutation).then(function (res) {
      return callIfExists(options.onComplete, res);
    });
  });
};
...
HanziWriter.prototype.quiz = function () {
  ...
  this._withData(function () {
    _this10.cancelQuiz();
    _this10._quiz = new Quiz(_this10._character, _this10._renderState, _this10._positioner);
    _this10._quiz.startQuiz(assign({}, _this10._options, quizOptions));
  });
};
...
HanziWriter.prototype.setCharacter = function (char) {
  ...
  this._withDataPromise = this._loadingManager.loadCharData(char).then(function (pathStrings) {
    ...
    _this11._character = parseCharData(char, pathStrings);
...
HanziWriter.prototype._withData = function (func) {
  ...
  return this._withDataPromise.then(function () {
    if (!_this12._loadingManager.loadingFailed) {
      return func();
    }
  });
};
...
HanziWriter.prototype._setupListeners = function () {
  ...
  this.target.addPointerStartListener(function (evt) {
    if (_this13.isLoadingCharData || !_this13._quiz) return;
    evt.preventDefault();
    _this13._forwardToQuiz('startUserStroke', evt.getPoint());
  });
  this.target.addPointerMoveListener(function (evt) {
    if (_this13.isLoadingCharData || !_this13._quiz) return;
    evt.preventDefault();
    _this13._forwardToQuiz('continueUserStroke', evt.getPoint());
  });
  this.target.addPointerEndListener(function () {
    return _this13._forwardToQuiz('endUserStroke');
  });
};
